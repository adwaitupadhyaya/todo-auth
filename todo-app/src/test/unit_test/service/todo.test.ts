import * as TodosModel from "../../../model/todo";
...
describe("Todos Service Test Suite", () => {
  ...
  describe("createTodos", () => {
    it("should create a new todo", () => {
      ...
      const todoModelCreateTodoStub = sinon
        .stub(TodosModel, "createTodo")
        .returns(createdTodo);

      const result = createTodo(newTodo, userId);

      expect(todoModelCreateTodoStub.calledOnceWith(newTodo, userId)).toBe(
        true
      );
  ...
  describe("updateTodo", () => {
    ...
    it("should update an existing todo", () => {
      ...
      const response = updateTodo(todoId, updatedTodo as ITodo, userId);

      expect(todoModelGetTodoByIdStub.calledOnceWith(todoId, userId)).toBe(
        true
      );
      expect(todoModelUpdateTodoStub.calledOnceWith(todoId, updatedTodo)).toBe(
        true
      );
    ...
    it("should return undefined when todo does not exist", () => {
      ...
      const response = updateTodo(todoId, todo as ITodo, userId);
